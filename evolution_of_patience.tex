...
Intertemporal choices are not only made by individuals. For instance,  
environmental policy dictates whether to reduce exploitation in the present to preserve resources for future use. 
...
As a concrete example, suppose chocolate provides benefit $1$ at delay $0$, whereas fruit salad provides benefit $2$ at delay $10$. 
...
Appreciation is the reciprocal of discounting, and either may be used to describe time preferences depending on which more naturally suits our application. 
...
\end{equation}
which grows linearly in time $t$. Figure \ref{fig:discounting_appreciation} graphically depicts both the exponential and hyperbolic discounting models. 
...
The imposition of a common delay before the consumption of chocolate or fruit salad significantly impacts some individuals' preferences. 
Some participants prefer chocolate if consumption is immediate and prefer fruit salad if consumption is after one week. 
...
We propose to use techniques from evolutionary modeling to resolve the discrepency between the theory and the practice of time preferences.
...
time preferences is called the \textit{adaptive unconscious}, which is the collection of mental processes used when making 
...
Our approach reveals that the demographic factors survival and ambient population growth determine risk and opportunity cost, respectively. 
...
Assuming discrete age classes is not limiting since we may take units of time small enough so as to closely mirror continuous time. Further,
