...
\section{Life Histories, Reproductive Success, and Fitness}
...
The baseline life history $\LH_0$ is the same for all individuals. A variable environment presents
individuals with fecundity dilemmas,  
a choice between a small increase to $b_a$ and a larger increase to $b_{a+1}$. 
An individual's life history is the baseline $\LH_0$ in addition to either the smaller/sooner ($\smallersooner$)
...
age at which the dilemma is faced and $F > 0$ is the ratio of $\largerlater$ to $\smallersooner$. Discounting in a life history 
context requires a slightly different approach to discounting in general. Rather than discounting delayed rewards
to present value, additional fecundity at age $a$ is discounted to its value at age $0$. Notationally, $\Delta(a)$ is interpreted as
the discount factor comparing fecundity at age $a$ to fecundity at age $0$. 
...
%measures from ecology: lifetime reproductive output $R_0$ and monomorphic population growth rate $\lambda$ 
%\cite{stearns92}.
%


An individual's lifetime reproductive output is computed as
\begin{equation}
